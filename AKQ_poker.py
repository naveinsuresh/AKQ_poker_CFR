...
class AKQ:
    ...
    def cfr(self, history, p1_reach, p2_reach, sample_reach):
        ...
        if player == 0:
            util, p_tail = self.cfr(
                next_history,
                p1_reach * node.strategy[act],
                p2_reach,
                sample_reach * probability[act],
            )
        else:
            util, p_tail = self.cfr(
                next_history,
                p1_reach,
                p2_reach * node.strategy[act],
                sample_reach * probability[act],
            )
        util *= -1
        ...
        if player == self.current_player:
            ...
            for a in range(len(strategy)):
                regret = (
                    W * (1.0 - strategy[act]) * p_tail
                    if a == act
                    else -W * p_tail * strategy[act]
                )
    ...
    def sample_strategy(self, strategy):
        for i in range(len(strategy)):
            strategy[i] = (
                self.epsilon * np.repeat(1 / self.n_actions, self.n_actions)[i]
